...
class ViewLabel extends ViewFormat {
  ...
  ViewLabel (int width, int charWidth, int horizontalAlignment, Font font, Color color, HighlightControl highlightControl, List <Format> formats) {
    super (formats);
    prototype       = new JLabel ();
    baseCharWidth   = prototype.getFontMetrics (font).charWidth ('W');
    if (width==0)
      width = charWidth * baseCharWidth;
    isWidthVariable = width < 0;
    baseWidth       = Math.abs (width);
    Dimension sz    = new Dimension (baseWidth, prototype.getPreferredSize ().height);
  ...
  ViewLabel (int width, int charWidth, int horizontalAlignment, Font font, Color color, HighlightControl highlightControl, Format format) {
    this (width, charWidth, horizontalAlignment, font, color, highlightControl, Arrays.asList (format));
  }
